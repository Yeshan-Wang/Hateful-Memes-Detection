...
The folder contains the complete experiments for feature extraction by image captioning. Firstly, we perform data pre-processing to detect and remove texts from meme images and save the clean images for feature extraction. Then we apply pre-trained image captioning model [BLIP](https://huggingface.co/Salesforce/blip-image-captioning-large) to generate textual description of clean images. The generated captions will be saved as corresponding CSV file for integrating image captioning features as additional textual inputs of the CLIP model for hateful memes classification. However, upon careful inspection, some [generated captions](https://github.com/Yeshan-Wang/Hateful-Memes-Detection/blob/main/4.%20Feature%20Extraction%20by%20Image%20Captioning/BLIP/BLIP_caption.csv) were found to be of poor quality. For example, the model generated an exception word ”arafed” at the beginning of some captions, which appears 2,734 times in total. Such erroneous captions could potentially introduce bias and noise into our classification model, thereby influencing its accuracy and performance.   

To address this issue, we employed another pre-trained image captioning model [BLIP-2](https://huggingface.co/Salesforce/blip2-opt-2.7b-coco) for captions generation, which is an upgraded version of the [BLIP](https://huggingface.co/Salesforce/blip-image-captioning-large) model. It leverages a decoder-based large language model [OPT](https://huggingface.co/facebook/opt-2.7b) with 2.7 billion parameters to help generate higher quality captions. We then proposed two distinct models for integrating extracted caption features:

- **CLIP-caption-unimodal:** This model transforms the dual modalities (image and text) of a meme into unimodal textual representations by replacing the image component with its corresponding caption.
- **CLIP-caption-multimodal:** This model utilizing the captioning features as supplementary inputs to the original classification system without removing the image component.

| Image Captioning Model | Validation AUROC | Test AUROC |  Colab Links  |
| ---------------------- | ---------------- | ---------- | ------------- |
| CLIP-caption-multimodal | 81.79 | 82.74 | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/17-GtNCnQoXtCIvyfexCj_fOlokKYRlOy) |
| CLIP-caption-unimodal | 72.86 | 73.99 | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/1Ch2Kdj03Wzs1YBA6l1DoLy6sp98aSqu3) |

## 5. Hyperparameter optimization + Ensemble Learning
The folder contains the complete experiments based on the CLIP-caption-multimodal model for hyper-parameters tuning over the dropout rate, learning rate, activation function, optimizer and scheduler type and save all best performing models based on the validation AUROC score. We then perform soft voting method for ensemble learning by averaging the predictions of best performing models.
...
- [CLIP-caption-multimodal model](https://github.com/Yeshan-Wang/Hateful-Memes-Detection/blob/main/6.%20Error%20Analysis/CLIP_caption_without_image.csv)
...
For each model, we show its confusion matrix separately, as well as comparing some mis-classified memes (False Positives, False Negatives) between different models, such as:
- Memes that only CLIP-baseline Model mis-classified
- Memes that only CLIP-baseline Model classified correctly
- Memes that only CLIP-caption-unimodal Model mis-classified
